...
- The `MonoVertex` panel draws the container chain (ADR 0010): one card for the unit, with `source ──▶ transform ──▶ map ──▶ sink` inside it and a dashed hop to the fallback sink, the replica badge in the top border and the rate and pending count in the bottom one. Labels abbreviate and the card floats in the middle of the panel rather than overflowing or sitting in a corner. `MonoVertex` gained `has_fallback`, read from `spec.sink.fallback`.
